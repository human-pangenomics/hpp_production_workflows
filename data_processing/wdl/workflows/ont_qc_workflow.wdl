...
workflow ont_qc_wf {
    ...
    output {
        ...
        File ont_ntsm_counts = ntsm_wf.ntsm_counts
    }
}
...
task summarize_ont_qc {
    ...
    command <<<
        ...
        echo -e "file_name\tpass_read_N50\tpass_Gb\tpass_coverage\tpass_100kb+\tpass_200kb+\tpass_300kb+\tpass_400kb+\tpass_500kb+\tpass_1Mb+\tpass_whales\tfail_read_N50\tfail_Gb\tfail_coverage\tfail_100kb+\tfail_200kb+\tfail_300kb+\tfail_400kb+\tfail_500kb+\tfail_1Mb+\tfail_whales" > "~{file_name}.summary.tsv"

        # Extract data from pass_summary, skip header
        pass_data=$(awk -F'\t' 'NR > 1 {print $2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10"\t"$11}' $pass_summary)

        # Extract data from fail_summary, skip header
        fail_data=$(awk -F'\t' 'NR > 1 {print $2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10"\t"$11}' $fail_summary)

        # Append data to the output file
        echo -e "$filename\t$pass_data\t$fail_data" >> "~{file_name}.summary.tsv"
    >>>
