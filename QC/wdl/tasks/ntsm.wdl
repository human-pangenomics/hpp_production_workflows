    ...
    command <<<
        ...
        for READFILE in "${READS[@]}"
            do
                ...
                if [[ "$SUFFIX" == "bam" ]] ; then
                    samtools fastq -@~{threadCount} $READFILE > output/${PREFIX}.fastq.gz
                    ...
                    ln -s ~{cram_reference}
                    samtools fastq -@~{threadCount} --reference `basename ~{cram_reference}` $READFILE > output/${PREFIX}.fastq.gz
