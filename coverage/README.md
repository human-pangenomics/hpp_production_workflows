...
1. In some cases the duplicated component is mixed up with the haploid component. It usually happens when the coverage in the haploid component drops systematically and the contig has long stretches of false duplication. One extreme case happened in `HG002#1#JAHKSE010000028.1`. Nearly half of this contig is falsely duplicated but the current results report almost the whole contig as `duplicated`. (file:///home/mobin/Downloads/HG002.diploid.f1_assembly_v2_genbank.hifi.winnowmap_v2.03.cov_dist.pdf). One other indicator of a false duplication is the accumulation of alignments with MAPQ 0. We will use produce another coverage file only for MAPQ 0 alignments and intersect it with the current results to correct them.
